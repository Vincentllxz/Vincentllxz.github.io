...
![Transport Layer action 1](/images/cn/ch03-transport/transport-layer-action-1.png)
- **Receiver**: Check header value，然後 Demultiplex message 到正確的 process
![Transport Layer action 2](/images/cn/ch03-transport/transport-layer-action-2.png)
...
![Multiplexing & Demultiplexing 1](/images/cn/ch03-transport/multiplexing-and-demultiplexing-1.png)
- **demultiplexing**: 解讀 header 傳送到正確的 socket
![Multiplexing & Demultiplexing 2](/images/cn/ch03-transport/multiplexing-and-demultiplexing-2.png)


### Multiplexing & Demultiplexing for UDP
- header 的架構必須有這些東西
![Multiplexing & Demultiplexing for UDP 1](/images/cn/ch03-transport/multiplexing-and-demultiplexing-for-udp-1.png =50%x)
...
![Multiplexing & Demultiplexing for UDP 2](/images/cn/ch03-transport/multiplexing-and-demultiplexing-for-udp-2.png)
...
- 一樣交換兩端資訊即可送回
![Multiplexing & Demultiplexing for TCP](/images/cn/ch03-transport/multiplexing-and-demultiplexing-for-tcp.png)
...
![Use of UDP 1](/images/cn/ch03-transport/use-of-udp-1.png)
   ![Use of UDP 2](/images/cn/ch03-transport/use-of-udp-2.png)
</Aside>
...
- **Checksum**
![UDP structure](/images/cn/ch03-transport/udp-structure.png =50%x)
...
- 做 complement 
![UDP Check Sum 1](/images/cn/ch03-transport/udp-check-sum-1.png)
</Aside>

<Aside type="caution" title="Checksum is a WEAK PROTECTION">
- 在這個例子裡 Checksum 沒變但裡面被 flip 了
![UDP Check Sum 2](/images/cn/ch03-transport/udp-check-sum-2.png)
</Aside>

## Reliable Data Transfer (RDT)
我們想像中是左圖，但實際上是右邊，並沒有真正的 reliable channel  
![Reliable Data Transfer 1](/images/cn/ch03-transport/reliable-data-transfer-1.png)
- 要考慮非常複雜的狀況，要站在另外一端的角度思考會發生什麼事情
    - 要設計出一對這樣的 protocol 而且要可以雙向溝通
- 實際上的 interface 其實長這樣
![Reliable Data Transfer 2](/images/cn/ch03-transport/reliable-data-transfer-2.png)
- 我們後面都用這種形式來表示狀態轉移
![Reliable Data Transfer 3](/images/cn/ch03-transport/reliable-data-transfer-3.png)
...
![rdt 1.0: Reliable transfer with Reliable channel](/images/cn/ch03-transport/rdt-1-0-reliable-transfer-with-reliable-channel.png)
...
- 上述過程可以畫成這個 FSM 
![rdt 2.0: Channel with bits error](/images/cn/ch03-transport/rdt-2-0-channel-with-bits-error.png)
...
![rdt 2.1: Handling Garble ACKs/NAKs 1](/images/cn/ch03-transport/rdt-2-1-handling-garble-acks-naks-1.png)
- **Receiver**:
    - 當收到並非自己 sequence number 的時候，也要在原地等待，並且送出 `ACKs` 的資訊
    - 要檢查是否是自己的 sequence number
![rdt 2.1: Handling Garble ACKs/NAKs 2](/images/cn/ch03-transport/rdt-2-1-handling-garble-acks-naks-2.png)

### rdt 2.2: Free NAKs Protocol
- 其實 `NAK` 可以用 `udt_send()` 非自己 sequence number 來取代
- 可以簡化 FSM 成
![rdt 2.2: Free NAKs Protocol](/images/cn/ch03-transport/rdt-2-2-free-naks-protocol.png)
...
- 要有 timeout checker 來檢查是否超過時間，超過則重新發送 pkt
   ![rdt 3.0 channels with errors and loss](/images/cn/ch03-transport/rdt-3-0-channels-with-errors-and-loss.png)
   - `star_timer` 於發送 pkt 開始計算
   - `stop_timer` 於 `is_ACK()` 發送 

#### Action of rdt 3.0
![Action of rdt 3.0 1](/images/cn/ch03-transport/action-of-rdt-3-0-1.png)
![Action of rdt 3.0 2](/images/cn/ch03-transport/action-of-rdt-3-0-2.png)
- **Case (a)**: No loss
- **Case (b)**: `pkt` loss 
...
![Performance of rdt 3.0](/images/cn/ch03-transport/performance-of-rdt-3-0.png)
...
- 允許一次多個 sequence number 的同時傳送
    ...
    - 需要更多 buffer space
![Pipeline 1](/images/cn/ch03-transport/pipeline-1.png)

<Aside type="tip" title="Example">
![Pipeline 2](/images/cn/ch03-transport/pipeline-2.png)
...
**Sender**
![Go-Back-N sender/receiver 1](/images/cn/ch03-transport/go-back-n-sender-receiver-1.png)
上圖是一整串 `pkt`
- 名詞
    - 綠：已經被 `ACK`
    - 黃：送出但尚未 `ACK`
...
- FSM of Sender
    ![Go-Back-N sender/receiver 2](/images/cn/ch03-transport/go-back-n-sender-receiver-2.png)

**Receiver**
![Go-Back-N sender/receiver 3](/images/cn/ch03-transport/go-back-n-sender-receiver-3.png)
- 需要紀錄 `recv_base`，也就是下一個要收到的 **in-order** `pkt`
- 必須接收 in-order `pkt`，若遇到不是 `in-order` 就 discard，不停發送 `ACK(recv_base - 1)`

<Aside type="tip" title="Example">
![Go-Back-N sender/receiver 4](/images/cn/ch03-transport/go-back-n-sender-receiver-4.png)
...
![Selective Repeat 1](/images/cn/ch03-transport/selective-repeat-1.png)
...
<Aside type="tip" title="Example">
![Selective Repeat 2](/images/cn/ch03-transport/selective-repeat-2.png)
</Aside>

<Aside type="note" title="Dilemma of SR">
![Selective Repeat 3](/images/cn/ch03-transport/selective-repeat-3.png)
...
![TCP segment structure](/images/cn/ch03-transport/tcp-segment-structure.png)
...
![TCP seq \# 1](/images/cn/ch03-transport/tcp-seq-1.png)

<Aside type="tip" title="Example">
![TCP seq \# 2](/images/cn/ch03-transport/tcp-seq-2.png)
...
$$\text{EstimatedRTT} = (1-\alpha)*\text{EstimatedRTT} + \alpha \times \text{SampleRTT}$$
    ...
    - ![Timer](/images/cn/ch03-transport/timer.png)
...
<Aside type="tip" title="Example">
![Sender event](/images/cn/ch03-transport/sender-event.png)
...
![fast retransmit](/images/cn/ch03-transport/fast-retransmit.png)
...
![TCP Flow Control](/images/cn/ch03-transport/tcp-flow-control.png)

### TCP connection managment
- **"handshake"** 要讓兩邊的 data sync 一下
    - initial, `rwnd`, 是否 accept, `seq #`
![TCP connection managment](/images/cn/ch03-transport/tcp-connection-managment.png)
...
![3-way handshake 1](/images/cn/ch03-transport/3-way-handshake-1.png)
1. server 關閉，但又建立了連線
2. dup data 的調整
</Aside>


![3-way handshake 2](/images/cn/ch03-transport/3-way-handshake-2.png)
...
- no-retransmission
![Senario 1 1](/images/cn/ch03-transport/senario-1-1.png)
![Senario 1 2](/images/cn/ch03-transport/senario-1-2.png)
...
![Senario 2 1](/images/cn/ch03-transport/senario-2-1.png)
![Senario 2 2](/images/cn/ch03-transport/senario-2-2.png)
- $\lambda'_\text{in}$ 傳送 $R/2$ 的資料但裡面有一些是重複無效資料，因此 $\lambda_\text{out}$ 達不到 $R/2$
![Senario 2 3](/images/cn/ch03-transport/senario-2-3.png)
...
![Senario 3 1](/images/cn/ch03-transport/senario-3-1.png)
![Senario 3 2](/images/cn/ch03-transport/senario-3-2.png)
...
- 出現 `loss` 後減少 sending rate 一半
    - 砍一半
![AIMD](/images/cn/ch03-transport/aimd.png)
...
![TCP congetion window](/images/cn/ch03-transport/tcp-congetion-window.png)
...
![TCP slow start](/images/cn/ch03-transport/tcp-slow-start.png)
...
![FSM of TCP congestion control](/images/cn/ch03-transport/fsm-of-tcp-congestion-control.png)
...
![Explicit Congestion Notification](/images/cn/ch03-transport/explicit-congestion-notification.png)
- 只有某些 TCP 才能做這件事
...
![TCP Fairness](/images/cn/ch03-transport/tcp-fairness.png)
