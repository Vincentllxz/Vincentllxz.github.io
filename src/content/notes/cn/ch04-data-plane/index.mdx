...
![Routing 1](/images/cn/ch04-data-plane/routing-1.png)
- 用統一的 distributed algorithm，router 之間的 algorithm 互相溝通形成 forwarding table
2. **Software-Defined Networking(SDN)**
![Routing 2](/images/cn/ch04-data-plane/routing-2.png)
...
![Service model](/images/cn/ch04-data-plane/service-model.png)
...
![Router Architecture](/images/cn/ch04-data-plane/router-architecture.png)
...
![Input port](/images/cn/ch04-data-plane/input-port.png)
...
![Destination-based forwarding 1](/images/cn/ch04-data-plane/destination-based-forwarding-1.png)
...
![Destination-based forwarding 2](/images/cn/ch04-data-plane/destination-based-forwarding-2.png)
對於這個 table
...
![Switching fabrics 1](/images/cn/ch04-data-plane/switching-fabrics-1.png)

- Switching via **Memory**
    - First generation Routers
    - 這種 switching 就是另一種普通 I/O 而已
    ![Switching fabrics 2](/images/cn/ch04-data-plane/switching-fabrics-2.png)
- Switching via **Bus**
    - 從 input 輸入後，經由 bus connection 不經過 memory 直接前往 output
    - 限制就變成 bus 的頻寬
    ![Switching fabrics 3](/images/cn/ch04-data-plane/switching-fabrics-3.png)
- Switching via **Interconnection Network**
    - 本來是設計給 multiprocessor 用的一種傳輸方法
    - 通常不會用上面的形式，而是用 **multi-stage** 的形式
    ![Switching fabrics 4](/images/cn/ch04-data-plane/switching-fabrics-4.png)
    - $8 \times 8$ 的設計但只需要 4 個中繼點就好了
    - 我們通常會想用 parellel 的形式去做傳送，可以讓速度大幅增加
    ![Switching fabrics 5](/images/cn/ch04-data-plane/switching-fabrics-5.png)
...
![Input Port Queue](/images/cn/ch04-data-plane/input-port-queue.png)
...
![Output Port Queue 1](/images/cn/ch04-data-plane/output-port-queue-1.png)
...
- **Packet Scheduling** 要決定誰先執行
    ...
    ![Output Port Queue 2](/images/cn/ch04-data-plane/output-port-queue-2.png) 
        - 就是種最基本的 FIFO 結構
    - **Priority**
    ![Output Port Queue 3](/images/cn/ch04-data-plane/output-port-queue-3.png)
        - 會根據 ISP 的 Priority 去做分配，如果 packet prioty 高就會被存在 high priority queue
        ![Output Port Queue 4](/images/cn/ch04-data-plane/output-port-queue-4.png)
        - 雖然 packet 2 比 packet 3 還早到，但他們在 packet 1 處理完成的時候都已經進入 queue，所以會優先執行 high priority 的
    - **Round Robin** (RR) scheduling
    ![Output Port Queue 5](/images/cn/ch04-data-plane/output-port-queue-5.png)
        - 一樣分成多個 queue，但這次執行順序是輪流，每個 queue equally 的輪流執行
    - **Weighted Fair Queuing (WFQ)**
    ![Output Port Queue 6](/images/cn/ch04-data-plane/output-port-queue-6.png)
...
以下是當代的 IP header 架構
![IP](/images/cn/ch04-data-plane/ip.png)
...
![Fragment, Reassemble 1](/images/cn/ch04-data-plane/fragment-reassemble-1.png)
...
![Fragment, Reassemble 2](/images/cn/ch04-data-plane/fragment-reassemble-2.png)
...
![IP addressing](/images/cn/ch04-data-plane/ip-addressing.png)
...
![Subnets](/images/cn/ch04-data-plane/subnets.png)
...
#### DHCP
![DHCP 1](/images/cn/ch04-data-plane/dhcp-1.png)
...
![DHCP 2](/images/cn/ch04-data-plane/dhcp-2.png)
...
![Hierarchical addressing 1](/images/cn/ch04-data-plane/hierarchical-addressing-1.png)

在 normal 情況下我們可以把 routing 存起來，告訴 internet 把 $\texttt{200.23.16.0/20}$ 全都傳給我

![Hierarchical addressing 2](/images/cn/ch04-data-plane/hierarchical-addressing-2.png)
...
![Hierarchical addressing 3](/images/cn/ch04-data-plane/hierarchical-addressing-3.png)
...
![NAT](/images/cn/ch04-data-plane/nat.png)
...
![IPv6 format](/images/cn/ch04-data-plane/ipv6-format.png)
...
![Tunneling](/images/cn/ch04-data-plane/tunneling.png)
...
![Forwarding](/images/cn/ch04-data-plane/forwarding.png)
