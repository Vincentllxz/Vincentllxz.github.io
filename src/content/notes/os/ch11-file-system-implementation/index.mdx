...
- **Volume control block** (per volume): 儲存 volume 的 metadat 用的 block
...
4. **Extent**，空間不夠厚分段，用 linklist 連接每個 segment
...
- Contiguous allocation 在 direct access 上很快，但會有 external feagmentation
...
有一個 list head 指向第一個 free block，把所有 block 用一個，相比於 bitmap 我們需要的只是 pointer，空間會少，而且找第一個 free block 會比較短時間

但缺點是 traverse list 依然是一個 time comsuming 的事情，但其實 traverse 的機會不多
...
這個改念跟 file-allocation table (FAT) 是很像的只不過我們記錄的是 free-block，所以假設我們有 FAT 可以串 free block，也可以不用 seperate method 來處理，只需要注記是在記錄 free block 的 list
...
Space map 會搭配 log-structure file system technique 來進行處理，簡而言之是會把 block 的 activity 存成一個 log，把 block load 到 memory 上之後再根據 log 進行一系列的 operation，就可以知道現在的狀態
