---
title: "NTU-OS 作業系統 Ch12 I/O Systems"
description: "NTU Operating System 作業系統:Ch12 I/O Systems"
...
- **PCIe (Peripheral Component Interconnect Express) Bus** 連接所有 Device, Memory
- **Expansion Bus** 可以讓慢速裝置傳輸稍微加速
- **Serial-attached SCSI (SAS)** bus 串接 DISK

<div style="text-align: center;">
  <img loading="lazy" decoding="async" src="/images/os/ch08-io-systems/typical-pc-bus-structure.webp" style="width: 70%;" />
  <br />
...
<div style="text-align: center;">
  <img loading="lazy" decoding="async" src="/images/os/ch08-io-systems/memory-device.webp" style="width: 70%;" />
  <br />
  <span style="color: grey; font-size: 16px;">
...
1. 一直 read `status Register` 裡面的 `busy` bit 直到變成 `clear` 了
2. Host set 一個 `write` bit 在 `command(control) register` 並且把資料放到 `data-out register` 
3. Host set `command-ready` bit 在 `command(control) register` 裡面
4. Controller set `busy` bit 在 `status register` 裡面
...
<div style="text-align: center;">
  <img loading="lazy" decoding="async" src="/images/os/ch08-io-systems/interrupt-driven-i-o-cycle.webp" style="width: 70%;" />
  <br />
  <span style="color: grey; font-size: 16px;">
...
<div style="text-align: center;">
  <img loading="lazy" decoding="async" src="/images/os/ch08-io-systems/interrupt-vector.webp" style="width: 100%;" />
  <br />
  <span style="color: grey; font-size: 16px;">
     Interrupt vector
  </span>
...
我們希望大量的 I/O 傳輸 **不要牽扯到 CPU** 也就是避免 **programmed I/O**（one byte by a time，由 CPU 下指令控制的 I/O）

<div style="text-align: center;">
  <img loading="lazy" decoding="async" src="/images/os/ch08-io-systems/dma-progress.webp" style="width: 100%;" />
  <br />
  <span style="color: grey; font-size: 16px;">
     DMA Progress
  </span>
...
2. Device driver 會告訴 Controller 要傳輸 `c` bytes 到 address `x` 上
...
- **DMA-request** and **DMA-acknowledge** wires：handshaking 時候的 DMA controller 跟 device 之間的 communication（就跟網路傳輸一樣，詳見[計算機網路 transport layer](https://vinsong.csie.org/notes/cn/ch03-transport)）
...
<div style="text-align: center;">
  <img loading="lazy" decoding="async" src="/images/os/ch08-io-systems/application-i-o-interface-1.webp" style="width: 100%;" />
  <br />
  <span style="color: grey; font-size: 16px;">
     Application I/O Interface structure
  </span>
</div>
...
<div style="text-align: center;">
  <img loading="lazy" decoding="async" src="/images/os/ch08-io-systems/application-i-o-interface-2.webp" style="width: 100%;" />
  <br />
  <span style="color: grey; font-size: 16px;">
     Application I/O Interface types
  </span>
</div>
...
**Block-device interface**，通常用在 disk 上，以 block 為單位進行讀寫，通常會有 `read()`, `write()`，由於是 Addressable 的所以還可以用 `seek()` 去做操作，每個 system call 都有不同的 parameter
- **Raw I/O**：跳過 OS 的 File System, OS buffer cache，直接去控制 I/O device，(e.g., Database system 對 Disk 有更好的 Management)
- **Direct I/O**：透過 OS 的 File System 操作檔案，但跳過 OS 的 [Buffer Cache / Page Cache](https://vinsong.csie.org/notes/os/ch11-file-system-implementation#buffer-cache-and-page-cache)，這個模式下會 disable buffer, locking

在這些操作上，可以有額外的 library 去做額外的變化，像是 memory-mapped file 的 access，利用 MMU, Page fault 去做一些優化，或是直接把檔案的內容映射到 memory 裡面，這樣就不需要 `read()`, `write()` 了
...
<div style="text-align: center;">
  <img loading="lazy" decoding="async" src="/images/os/ch08-io-systems/example-of-i-o-scheduling.webp" style="width: 100%;" />
  <br />
  <span style="color: grey; font-size: 16px;">
...
<div style="text-align: center;">
  <img loading="lazy" decoding="async" src="/images/os/ch08-io-systems/life-cycle-of-blocking-read-request-1.webp" style="width: 90%;" />
  <br />
  <span style="color: grey; font-size: 16px;">
...
<div style="text-align: center;">
  <img loading="lazy" decoding="async" src="/images/os/ch08-io-systems/life-cycle-of-blocking-read-request-2.webp" style="width: 40%;" />
  <br />
  <span style="color: grey; font-size: 16px;">
     Life Cycle of Blocking Read Request
  </span>
...
<div style="text-align: center;">
  <img loading="lazy" decoding="async" src="/images/os/ch08-io-systems/intercomputer-communications.webp" style="width: 90%;" />
  <br />
  <span style="color: grey; font-size: 16px;">
...
<div style="text-align: center;">
  <img loading="lazy" decoding="async" src="/images/os/ch08-io-systems/device-functionality-progression.webp" style="width: 70%;" />
  <br />
  <span style="color: grey; font-size: 16px;">
