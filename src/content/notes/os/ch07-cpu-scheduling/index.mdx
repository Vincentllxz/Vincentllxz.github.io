---
title: "NTU-OS 作業系統 Ch7 CPU Scheduling"
description: "NTU Operating System 作業系統:Ch7 CPU Scheduling"
...
1. **CPU Burst** 進行運算
2. **I/O Burst** 等帶 I/O event

我們稱為 **CPU-I/O Burst Cycle**，而從 System 的角度來看，必須讓 CPU 在某個 I/O Burst 的時間去做別的事情 

<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/cpu-schedule-concept.png" style="width: 50%;" />
  <br />
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/histogram-of-cpu-burst-time.png" style="width: 70%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
...
![CPU Scheduler](/images/os/ch07-cpu-scheduling/cpu-scheduler.png)
</Aside>
...
大部分現代 OS 大部分是 **Preemptive Scheduling**，但可能會產生 **Race condition**，因為有可能產生倍 Priority high 的 Process context switch 掉的情況

> 這個問題會在 Synchronization 的章節解決

### Dispatcher

負責把 Process 分配下去給 CPU run，包含 **Context Swithch**, **Switch to User mode** 等一系列工作

<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/workflow-of-dispatcher.png" style="width: 50%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
...
- Minimize **response time**：Request 到 submit 的時間
...
![First-Come, First-Served Scheduling 1](/images/os/ch07-cpu-scheduling/first-come-first-served-scheduling-1.png)
Waiting time 分別是 $P_1=0,\ P_2=24,\ P_3=27$, average 是 $\hat{t}=17$

- 接下來假設 arrival order 是 $P_2 \to P_3 \to P_1$ 會拿到
![First-Come, First-Served Scheduling 2](/images/os/ch07-cpu-scheduling/first-come-first-served-scheduling-2.png)
...
| **Burst Time**    | 6    | 8    |7 |3|
 ...
 ![Shortest-Job-First Scheduling](/images/os/ch07-cpu-scheduling/shortest-job-first-scheduling.png)
...
![Shortest-Remaining-Time-First Scheduling](/images/os/ch07-cpu-scheduling/shortest-remaining-time-first-scheduling.png)

$$
\hat{t} = [ (10 - 1 - 0) + (1 - 1) + (17 - 2) + (5 - 3) ] / 4 = 26 / 4 = 6.5
$$

> 如果 Nonpreemptive OS 在這種情況下（arrival time 不一樣），我們的 SFJ 不一定會是 Optimal
...
![Round-Robin Scheduling](/images/os/ch07-cpu-scheduling/round-robin-scheduling.png)

簡單來講，Avg Turnround time 比 SJF 高，但在 Response time 會比較好
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/quantum-to-avg-turnaround-time.png" style="width: 60%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
...
SFJ 其實也是一種 Priority Scheduling，我們通常講到 Priority 都是
$$
...
![Priority Scheduling](/images/os/ch07-cpu-scheduling/priority-scheduling.png)
...
![Priority Scheduling with Round-Robin](/images/os/ch07-cpu-scheduling/priority-scheduling-with-round-robin.png)
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/multilevel-queue-1.png" style="width: 40%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
     Multilevel Queue 
  </span>
...
![Multilevel Queue Scheduling](/images/os/ch07-cpu-scheduling/multilevel-queue-scheduling.png)
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/multilevel-queue-2.png" style="width: 50%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
     Multilevel Queue 
  </span>
...
1. 如果一個 Process 先進到 $Q_0$，用 RR 做 schedule，在 8 個 quantum 內沒做完的話，我們就放到 $Q_1$ 內
2. 進到 $Q_1$，用 RR 做 schedule，在 16 個 quantum 內沒做完的話，我們就 Preempt $Q_2$ 內
3. 在 $Q_2$ 內做 FCFS
...
$$
\begin{cases}
P_2 \to P_3 \to P_1:[(4.4 - 0.4) + (5.4 - 1.0) + (13.4 - 0)]/3 = 7.27 \\ P_3 \to P_2 \to P_1:[(2.0 - 1.0) + (6 - 1.0) + (14 - 0)]/3 = 6.87\end{cases}
$$
...
- **Symmetric Multiprocessing (SMP)**: 每個 Processor 都是同等地位，每個 Processor 要自己去處理自己的 Schedule 問題，從 Queue 裡面拿出 task，可以分成兩種
    ...
    <div style="text-align: center;">
      <img src="/images/os/ch07-cpu-scheduling/share-queue.png" style="width: 50%;" />
      <br />
      <span style="color: grey; font-size: 14px;">
         Share Queue 
      </span>
    ...
    <div style="text-align: center;">
      <img src="/images/os/ch07-cpu-scheduling/seperate-queue.png" style="width: 50%;" />
      <br />
      <span style="color: grey; font-size: 14px;">
         Seperate Queue
      </span>
...
因為 core 在執行一個 thread 得時候一定會需要 **memory stall** 也就是需要去讀取 memory，就會需要暫停一下，為了增加效率，我們可以讓 thread 做交替的 Compute, Memory Stall，就可以達成加速

![Multicore Processors](/images/os/ch07-cpu-scheduling/multicore-processors.png)


在 OS 層面上他其實不在乎有多少個 Core 在執行我的 Process，或是硬體 implement 的方法，系統通常會做 **Virtulization**，也就是把真正的硬體，轉化成一個 OS 比較看得懂的形式
- 真正的把 CPU 對應到 Core 上的 Thread 我們是用 **Chip multithreading (CMT)** 來分配，在 Intel 裡面叫做 **hyper-threading**

<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/multithreaded-multicore-system.png" style="width: 50%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/os-view-of-multithreaded-multicore-system.png" style="width: 50%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
...
- User Thread，我們通常只會接觸這個，用 LWP 接到 Locgical processor 上
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/two-level-threading.png" style="width: 50%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
     Two level Threading
  </span>
...
我們可以分成兩種 Affinity
- **Soft affinity**: 不強迫，processor 可以拒絕，那就 schdule 到其他 core 上
- **Hard affinity**: 強迫他到那個 processor 上，可能是 security 的問題，在車子上可能有很多 CPU 但某些 task 一定要指定 CPU，因為車子通常是不做 schduling 的，東西都是週期性的
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/numa-system.png" style="width: 50%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
...
在做 realtime schdduling 的時候我們會引入 deadline 的概念
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/event-latency.png" style="width: 50%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
     Event latency
  </span>
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/interrupt-latency.png" style="width: 70%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
     Interrupt Latency
  </span>
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/dispatch-latency.png" style="width: 80%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
     Dispatch Latency
  </span>
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/periodic-process-require-cpu-at-constant-interva.png" style="width: 80%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/example-of-rate-monotonic-scheduling.png" style="width: 100%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/counter-example-of-rate-monotonic-scheduling.png" style="width: 100%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
...
> 雖然 $P_1$ 都可以符合，但 $P_2$ 失敗了，但其實明明就可以 fufill，只要不 Preempt $P_1$
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/example-of-edf-scheduling.png" style="width: 100%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
...
把 CPU 的運算時間，分成很多個小的 interval，保證分給某個 Task 某些數量的 interval，是一種 guarentee

假設 $T$ 是所有 Process share 的時間，Process $i$ 可以 share 到 $N_i$，也就是可以保證每個 Process 收到 $\frac{N_i}{T}$ 的 Time interval

假設 $T = 100$, $A, B, C$ 分別有 $50, 15, 20$，如果有個 $D$ 想進來，只要他 $> 15$ 就會被 admission controller 拒絕掉這個要求，因爲他不是照比例分的，而是時間保底，在 real-time system 裡面，我們做了數學分析，如果 $A$ 要做，我們保證他可以分道某個數量，那麼 worst case 就是**可控的**
...
早期的 Linux Schedling 是 $O(1)$ time，但是後來覺得應該要更優化，時間變長，但效果更好
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/example-of-domain.png" style="width: 50%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
      Example of domain
  </span>
...
![Windows scheduling](/images/os/ch07-cpu-scheduling/windows-scheduling.png)

> Real-time IDLE 還是比剩下的高

如果被放到 **foreground**，那通常他的 Priority 會被乘以 3，Win7 有提供 **user-mode scheduling (UMS)**
...
![Solaris scheduling](/images/os/ch07-cpu-scheduling/solaris-scheduling.png)

在 Solaris 裡面 Priority 數字越高，代表 Priority 越高

<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/solaris-priority.png" style="width: 50%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
      Solaris Priority
  </span>
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/fcfs-28ms.png" style="width: 80%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
      FCFS：28ms
  </span>
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/nonpreemptive-sjf-13ms.png" style="width: 80%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
...
<div style="text-align: center;">
  <img src="/images/os/ch07-cpu-scheduling/round-robin-23ms.png" style="width: 80%;" />
  <br />
  <span style="color: grey; font-size: 14px;">
      Round Robin：23ms
  </span>
